...
def main():
    logging.basicConfig(level=logging.INFO #设置日志输出格式
                    ,filename="226.log" #log日志输出的文件位置和文件名
    ...
    model = Transformer(seq_len=2048,vocab_size=53228,N=40,d_ff=3072,h=24)
    ...
    lr_optimizer = torch.optim.Adam(model.parameters(), lr=0.01)
    ...
    #torch_data=TensorDataset('./ldata/lData1-0.npy',if_dir=False)
    ...
    start = dic['step']
    loop = tqdm(enumerate(data_loader,start=start),total=len(data_loader))
    # print('-----------------------------------------------------\n\n',start,'-----------------------------------------------------\n\n')
    # for i,Input in loop:
    ...
    for i,Input in loop:
        ...
        logging.info(log)
        loop.set_postfix(loss=loss.item()*2048)
        model_engine.backward(loss)
        model_engine.step()
        lr_scheduler.step()
        if(i%2000==0):
